...
| Item | Donor path | Why needed | Rename / clean / minimize | FinalScan location | Status |
|---|---|---|---|---|---|
| `IUnityGraphicsVulkanV2` intercept + `vkCreateDevice` hook + safe-family queue injection | `US` `Runtime/Telemetry/Native/MerkabaVulkanTimestamps.cpp:508-917, 2609-2630` | only proven way to get a second queue on Unity's device | split into `Native/device/*.cpp`; add OS-style fallback; never force extensions | `Native~/src/vulkan_intercept.cpp`, `device_patch.cpp` | PORTED (C01/C02) |
| Fallback to original `vkCreateDevice` when injection fails | `OS` `Runtime/SigmaPrism/.../SigmaVulkanTimestamps.cpp:539-560` | app must start on any driver | pattern | same | PORTED (C02) |
| Frame-paced sliced jobs (`RecordJobSlice`, `SubmitNextSlice`) | `OS` `SigmaVulkanTimestamps.cpp:2392, 2561` | P4 COLD quanta without stalls | pattern → scheduler continuation cursor | `Native~/src/host/executor/sched_core.cpp` (bounded quanta, deferral instead of backlog) | PORTED (C02) |
| Per-owner timestamp query ranges (`f2cfe26`) | `SS` `Runtime/Telemetry/MerkabaGpuTimestamps.cs`, native query pools | correct multi-owner GPU timing | pattern | `Native~/src/host/executor/executor.cpp` (per-slot query pools, frame marks, frame stages) | PORTED (C02) |
| Job lifecycle invariants (fence creation before resource access, prepare ownership, atomic error publication) | `US` `9cfed011` (cpp:1234-1247, 2075-2108, 2827-2924) | SIGSEGV / use-after-free classes | invariants only (VULKAN_DONOR_LEDGER §5); code shape rejected | `Native~/src/host/executor/executor.cpp` (SubmitJob / RetireLocked) | PORTED (C02) |
| SPIR-V reflection → generated binding/offset tables, cross-checked against C# and HLSL constants | `US` `Tools/unity/generate_merkaba_native_executor_shaders.py:319-401, 623-667` | single source of truth for ABI (no literal offsets) | rewrite: emits C header + C# constants + validates kernel envelope (§15.6); push constants instead of uniform-name hashing | `Tools~/native/gen_abi.py` (header → GLSL + C#) | PORTED (C04) |
| Pipeline binary CAPTURE / BINARY_ONLY workflow + Java ContentProvider bootstrap loading the `.so` before UnityPlayer | `US` `Runtime/Telemetry/Native/MerkabaPipelineBinary.h`, `Runtime/Plugins/Android/MerkabaPipelineBootstrap.java`, `Tools/unity/generate_merkaba_pipeline_pack.py`, `MERKABA_PIPELINE_DELIVERY.md` | pre-warmed pipelines keyed by `pipelineBinaryUUID` | optional feature: `VkPipelineCache` + warm-up screen fallback; no forced extensions; no "no SPIR-V fallback" | `Native~/src/host/executor/pipeline_store.cpp` (cache + binaries, never forced) | PORTED (C02; capture bundle in C30) |
| C# ↔ native job descriptor validated by `structSize` + `abiVersion` | `US` `Runtime/Telemetry/MerkabaNativeVulkanExecutor.cs`, cpp:2735-2739 | ABI drift detection | keep principle, generated constants | `Runtime/Platform/Native/FinalScanHostNative.cs` (structSize + AbiVersion, generated WorldAbi.cs) | PORTED (C02) |
| Async readback completion tokens instead of `GraphicsFence` (`ba286ac`, `57ab534`) | `SS` `Runtime/Merkaba/MerkabaGrid.Storage.cs`, `MerkabaIntegrator.cs` | `AsyncQueueSynchronisation` fences throw on Quest | pattern for the C# side only | not needed: the C# side never fences (reads the last published FRONT, §11) | REJECTED (C05) |
...
| Item | Donor path | Why needed | Rename / clean / minimize | FinalScan location | Status |
|---|---|---|---|---|---|
| Controller ray shader + driver (hands as pose source, UI-wins raycast) | `SS` `Runtime/UI/ControllerRay.shader`, `ControllerRayDriver.cs` (329 lines), `VRDocumentRaycaster.cs` | interaction | rename | `Runtime/UI/ControllerRayDriver.cs`, `VRDocumentRaycaster.cs`, `Shaders/FinalScanControllerRay.shader` | PORTED (C22) |
| World-space UI Toolkit panel + follower | `SS` `Runtime/UI/DebugMenu.uxml/.uss`, `DebugMenuController.cs` (1390 lines), `DebugMenuFollower.cs` | panel host | production panel per FEATURE_PARITY §9; diagnostics separate document, hidden by default | `Runtime/UI/ScanPanel.uxml/.uss`, `ScanPanelController.cs`, `ScanPanelFollower.cs` | PORTED (C22, panel grows per FEATURE_PARITY) |
| Input mapping (thumbstick click menu, trigger select, grip erase) | `SS` `Runtime/RoomScanInputHandler.cs` (87 lines) | parity | rename | `Runtime/UI/InputMap.cs` | PORTED (C22) |
