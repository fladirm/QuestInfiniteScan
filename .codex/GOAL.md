...
law and frozen query-boundary schemas with explicit provenance:
...
N1R must derive the no-manifestation representative `ZEmpty`, prove exact
unbacked/explicit/NULL representation parity, and prove that an all-default
neighbourhood yields reducer identity, `DEFAULT_SAT` and no active work. Changing
supported state to `ZEmpty` remains a physical mutation that may reveal farther
first-hit support. Current `G z_null = 0` is not sufficient.

Coordinate-local relation support is the sparse fast path, not the complete
ontology. Descriptor-proven seam/gauge/nonlocal tuples may be generated from
`Psi + M`; they and their caches are disposable and never form a canonical graph.

N3R is the one publication-capable live cutover: sensor pullback and all required
native relations close before one delta can publish. There is no sensor-only
...
gradient solver. Each sampled locality retains an exact feasible-set/coupled-factor
certificate, optionally compressed as a proved directional information form.
Compatible evidence can only retain/tighten it; weaker evidence never broadens it,
conflict never averages, and no scalar confidence or physical cell world exists.
...
Active node: S4‑08, replacement run S4‑08.6 N0R–N7R. Authorities:

- [`S4-08.6_NATIVE_CLOSURE_PLAN.md`](S4-08.6_NATIVE_CLOSURE_PLAN.md)
