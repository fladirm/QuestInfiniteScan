...
Source digest: `402d1527bbddec3d97ce66ca29f78fbb5b4c7cef291f00834969261374770b91`

Scope: 161 files, 2192 symbols, 1627 functions/methods, 182 GPU kernels, 17 event links.
...
- `RoomUnderstanding.AnchorsChanged` → `OnMrukAnchorsChanged` ([Runtime/Core/RoomScanner.cs:826](../../Runtime/Core/RoomScanner.cs#L826))
...
- `PrismFilmSpawner.FilmsMutated` → `OnFilmsMutated` ([Runtime/Prism/Geometry/PrismMeshletBuilder.cs:200](../../Runtime/Prism/Geometry/PrismMeshletBuilder.cs#L200))
...
`ApplyRenderMode` (method, L850), `Awake` (method, L204), `CacheComponents` (method, L249), `ClearAllDataAsync` (method, L704), `ClearScan` (method, L687), `CompleteRoomStartup` (method, L316), `ConfigurePrismChunk` (method, L531), `CreateScanAnchorAsync` (method, L470), `CycleRenderMode` (method, L753), `GetActiveCameraProvider` (method, L861), `IsModeAvailable` (method, L777), `ObserveStart` (method, L650), `OnDisable` (method, L323), `OnMrukAnchorsChanged` (method, L836), `OnRoomAnchorReady` (method, L240), `PausePrismForResidency` (method, L520), `PausePrismPipeline` (method, L603), `PopulateSceneObjectRegistry` (method, L813), `PreparePrismPipeline` (method, L545), `ReleaseScanResources` (method, L671), `ResumePrismAfterResidency` (method, L526), `RoomScanner` (class, L44), `ScanLifecycleState` (enum, L28), `ScanRenderMode` (enum, L13), `SetCameraProvider` (method, L797), `SetRenderMode` (method, L737), `SetSafeShaderDefaults` (method, L842), `SetSceneObjectRegistry` (method, L803), `ShutdownPrismPipeline` (method, L613), `Start` (method, L212), `StartPrismCapture` (method, L588), `StartScanningAsync` (method, L383), `StartScanningCoreAsync` (method, L396), `StopScanning` (method, L486), `SubscribeToAnchorsChanged` (method, L822), `ToggleDebugMenu` (method, L789), `ToggleScanning` (method, L638), `UnsubscribeFromAnchorsChanged` (method, L829), `Update` (method, L331)
...
`ContinuationEvidenceFlags` (enum, L97), `ContinuationEvidenceGpu` (struct, L259), `CrossChunkTopologyPortalFlags` (enum, L60), `CrossChunkTopologyPortalGpu` (struct, L328), `Dispose` (method, L517), `ElasticChartStateGpu` (struct, L286), `EvidenceAlignedSplitPlanGpu` (struct, L301), `FilmMembershipFlags` (enum, L43), `FilmMembershipGpu` (struct, L142), `FrontierLoopFlags` (enum, L85), `FrontierLoopGpu` (struct, L234), `NextPowerOfTwo` (method, L595), `PressureManifoldDiagnostic` (enum, L12), `PressureManifoldFlags` (enum, L33), `PressureManifoldHeaderGpu` (struct, L112), `PressureManifoldPool` (class, L358), `SupportContourPageGpu` (struct, L184), `SupportContourSegmentGpu` (struct, L164), `SurfaceHalfEdgeFlags` (enum, L72), `SurfaceHalfEdgeGpu` (struct, L203), `SurfaceHalfEdgeRelation` (enum, L49)
...
`Bind` (method, L351), `EnsurePlanBuffers` (method, L582), `LateUpdate` (method, L227), `OnDestroy` (method, L214), `OnFilmsMutated` (method, L236), `PrismMeshletBuilder` (class, L15), `RequestBuild` (method, L253), `StartBuilding` (method, L144), `StopBuilding` (method, L206), `TryBuild` (method, L254), `TryIncrementalPublication` (method, L307), `TryInitialPublication` (method, L272)
...
`BindBoundaryCurveResources` (method, L669), `BindContourResources` (method, L517), `BindElasticResources` (method, L754), `BindHalfEdgeResources` (method, L578), `BindPortalResources` (method, L832), `CeilDiv` (method, L877), `CeilLog2` (method, L880), `DispatchAtlas` (method, L315), `FindKernels` (method, L439), `OnDestroy` (method, L292), `PrepareRestoredResidency` (method, L305), `PrismPressureManifoldAtlas` (class, L14), `ResetEmptyResidency` (method, L294), `SetChunkFrame` (method, L287), `StartAtlas` (method, L227), `StopAtlas` (method, L280)
...
`ActiveEdge` (gpu_function, L110), `BoundaryHashEntry` (struct, L8), `BoundaryHashKey` (gpu_function, L64), `ClaimBoundaryHalfEdges` (gpu_function, L248), `ClearHalfEdgeBoundaryClaims` (gpu_function, L242), `CommitBoundaryCurves` (gpu_function, L310), `FillCurveCache` (gpu_function, L188), `FilmPoint` (gpu_function, L73), `FilmUv` (gpu_function, L78), `FindBoundaryHalfEdge` (gpu_function, L131), `HashCell` (gpu_function, L53), `InsertBoundaryHash` (gpu_function, L216), `PointSegmentDistance` (gpu_function, L124), `Segment` (gpu_function, L186), `UvCubic` (gpu_function, L101), `WorldCell` (gpu_function, L62), `WorldControl` (gpu_function, L86), `WorldCubic` (gpu_function, L92), `if` (gpu_function, L435), `ClaimBoundaryHalfEdges` (gpu_kernel, L2), `ClearHalfEdgeBoundaryClaims` (gpu_kernel, L1), `CommitBoundaryCurves` (gpu_kernel, L3)

### `Runtime/Resources/Prism/ChunkStage.compute`

`ClearFilmRemap` (gpu_function, L189), `ClearPageRemap` (gpu_function, L288), `ContactBoundaryHeader` (struct, L28), `ContactFilmHeader` (struct, L13), `ContactMeshletDescriptor` (struct, L71), `ContactMeshletVertex` (struct, L62), `ContactTopologyEvidence` (struct, L54), `CopyBasePages` (gpu_function, L342), `CopyMicroPages` (gpu_function, L371), `DisplacementCell` (struct, L46), `DisplacementPageHeader` (struct, L37), `FilmSlotState` (struct, L81), `FinalizeChunkStage` (gpu_function, L478), `IndexBasePages` (gpu_function, L299), `IndexMicroPages` (gpu_function, L320), `PatchFilmDisplacement` (gpu_function, L416), `PrepareChunkStage` (gpu_function, L154), `StageBoundaries` (gpu_function, L257), `StageFilms` (gpu_function, L196), `StageMeshlets` (gpu_function, L431), `ClearFilmRemap` (gpu_kernel, L2), `ClearPageRemap` (gpu_kernel, L5), `CopyBasePages` (gpu_kernel, L8), `CopyMicroPages` (gpu_kernel, L9), `FinalizeChunkStage` (gpu_kernel, L12), `IndexBasePages` (gpu_kernel, L6), `IndexMicroPages` (gpu_kernel, L7), `PatchFilmDisplacement` (gpu_kernel, L10), `PrepareChunkStage` (gpu_kernel, L1), `StageBoundaries` (gpu_kernel, L4), `StageFilms` (gpu_kernel, L3), `StageMeshlets` (gpu_kernel, L11)

### `Runtime/Resources/Prism/ChunkTopologyStage.compute`

`PrepareTopologyStage` (gpu_function, L69), `StageBoundaryTopology` (gpu_function, L173), `StageContourPages` (gpu_function, L132), `StageContourTopology` (gpu_function, L144), `StageCrossChunkPortals` (gpu_function, L188), `StageFilmTopology` (gpu_function, L107), `StageFrontierLoops` (gpu_function, L164), `StageManifoldHeaders` (gpu_function, L99), `StagedFilmIsActive` (gpu_function, L61), `PrepareTopologyStage` (gpu_kernel, L1), `StageBoundaryTopology` (gpu_kernel, L7), `StageContourPages` (gpu_kernel, L4), `StageContourTopology` (gpu_kernel, L5), `StageCrossChunkPortals` (gpu_kernel, L8), `StageFilmTopology` (gpu_kernel, L3), `StageFrontierLoops` (gpu_kernel, L6), `StageManifoldHeaders` (gpu_kernel, L2)
...
`ContactTopologyEvidence` (struct, L4), `CreateChild` (gpu_function, L135), `CreateEvidenceAlignedChildren` (gpu_function, L187), `LoadG` (gpu_function, L47), `LoadH` (gpu_function, L38), `PartitionInformation` (gpu_function, L97), `PartitionSupportMask` (gpu_function, L79), `StoreG` (gpu_function, L67), `StoreH` (gpu_function, L56), `UpperIndex` (gpu_function, L33), `CreateEvidenceAlignedChildren` (gpu_kernel, L1)

### `Runtime/Resources/Prism/ContactComponentReduce.compute`

`AccumulateComponentExtents` (gpu_function, L406), `AccumulateComponentMoments` (gpu_function, L338), `AccumulateComponentPosterior` (gpu_function, L459), `AtomicAddPosterior` (gpu_function, L143), `BuildComponentDispatchArguments` (gpu_function, L331), `BuildComponentHash` (gpu_function, L260), `CandidateCell` (gpu_function, L105), `CandidateCount` (gpu_function, L87), `CholeskySolve` (gpu_function, L186), `ClearComponentPosterior` (gpu_function, L450), `ClearComponentProducts` (gpu_function, L242), `CompactComponentRoots` (gpu_function, L320), `Compatible` (gpu_function, L107), `ConeEvent` (struct, L22), `ContactFilmHeader` (struct, L32), `EvaluateComponentModel` (gpu_function, L590), `ExpandRejectedComponents` (gpu_function, L604), `FinalizeComponentExtents` (gpu_function, L430), `FinalizeComponentFrames` (gpu_function, L370), `HashCell` (gpu_function, L96), `HookComponentGraph` (gpu_function, L273), `InformationH` (gpu_function, L155), `Parent` (gpu_function, L126), `PosteriorValue` (gpu_function, L150), `PrepareComponentReduction` (gpu_function, L229), `RootAfterConvergence` (gpu_function, L131), `SafeNormalize` (gpu_function, L89), `SetInformationG` (gpu_function, L174), `SetInformationH` (gpu_function, L163), `ShortcutComponentParents` (gpu_function, L308), `SolveComponentPosterior` (gpu_function, L521), `UpperIndex` (gpu_function, L138), `AccumulateComponentExtents` (gpu_kernel, L10), `AccumulateComponentMoments` (gpu_kernel, L8), `AccumulateComponentPosterior` (gpu_kernel, L13), `BuildComponentDispatchArguments` (gpu_kernel, L7), `BuildComponentHash` (gpu_kernel, L3), `ClearComponentPosterior` (gpu_kernel, L12), `ClearComponentProducts` (gpu_kernel, L2), `CompactComponentRoots` (gpu_kernel, L6), `EvaluateComponentModel` (gpu_kernel, L15), `ExpandRejectedComponents` (gpu_kernel, L16), `FinalizeComponentExtents` (gpu_kernel, L11), `FinalizeComponentFrames` (gpu_kernel, L9), `HookComponentGraph` (gpu_kernel, L4), `PrepareComponentReduction` (gpu_kernel, L1), `ShortcutComponentParents` (gpu_kernel, L5), `SolveComponentPosterior` (gpu_kernel, L14)
...
`BuildSpawnTileDispatchArguments` (gpu_function, L280), `CandidatePublication` (struct, L71), `ClearCandidatePixelOwners` (gpu_function, L304), `ClearSpawnCandidates` (gpu_function, L287), `ClearSpawnTileState` (gpu_function, L232), `CompactSpawnTiles` (gpu_function, L245), `Compatible` (gpu_function, L204), `ConeEvent` (struct, L18), `ContactFilmHeader` (struct, L41), `EmitSpawnCandidates` (gpu_function, L311), `EnsurePressureManifold` (gpu_function, L809), `FilmSlotState` (struct, L79), `FinalizePressureManifoldCounts` (gpu_function, L1014), `IsSpawnCandidate` (gpu_function, L184), `LoadRay` (gpu_function, L167), `MapCandidatePixels` (gpu_function, L739), `ReserveCandidatePublication` (gpu_function, L861), `SafeNormalize` (gpu_function, L173), `SolveSymmetric3x3` (gpu_function, L215), `WriteCanonicalFilms` (gpu_function, L968), `WriteCanonicalMembership` (gpu_function, L989), `BuildSpawnTileDispatchArguments` (gpu_kernel, L3), `ClearCandidatePixelOwners` (gpu_kernel, L5), `ClearSpawnCandidates` (gpu_kernel, L4), `ClearSpawnTileState` (gpu_kernel, L1), `CompactSpawnTiles` (gpu_kernel, L2), `EmitSpawnCandidates` (gpu_kernel, L6), `EnsurePressureManifold` (gpu_kernel, L8), `FinalizePressureManifoldCounts` (gpu_kernel, L12), `MapCandidatePixels` (gpu_kernel, L7), `ReserveCandidatePublication` (gpu_kernel, L9), `WriteCanonicalFilms` (gpu_kernel, L10), `WriteCanonicalMembership` (gpu_kernel, L11)
...
`ActiveMeasuredSegment` (gpu_function, L69), `BootstrapManifoldIdentity` (gpu_function, L261), `BootstrapPortalGhosts` (gpu_function, L287), `BuildPortalReconcileArguments` (gpu_function, L303), `ClearPortalPlans` (gpu_function, L130), `CommitPortalPairs` (gpu_function, L187), `ContactViewState` (gpu_function, L84), `ExistingPortal` (gpu_function, L101), `FilmPoint` (gpu_function, L79), `HashCell` (gpu_function, L90), `PlanPortalCandidates` (gpu_function, L136), `PreparePortalBootstrap` (gpu_function, L253), `PreparePortalExport` (gpu_function, L118), `ReconcilePortalGhosts` (gpu_function, L310), `ReservePortalPairs` (gpu_function, L168), `SegmentDomain` (gpu_function, L64), `WorldCell` (gpu_function, L99), `BootstrapManifoldIdentity` (gpu_kernel, L7), `BootstrapPortalGhosts` (gpu_kernel, L8), `BuildPortalReconcileArguments` (gpu_kernel, L9), `ClearPortalPlans` (gpu_kernel, L2), `CommitPortalPairs` (gpu_kernel, L5), `PlanPortalCandidates` (gpu_kernel, L3), `PreparePortalBootstrap` (gpu_kernel, L6), `PreparePortalExport` (gpu_kernel, L1), `ReconcilePortalGhosts` (gpu_kernel, L10), `ReservePortalPairs` (gpu_kernel, L4)
...
`CachedSegment` (gpu_function, L52), `ChildHandle` (gpu_function, L99), `DisplacementCell` (struct, L13), `DisplacementPageHeader` (struct, L5), `InitializeSplitDisplacement` (gpu_function, L155), `LoadCell` (gpu_function, L105), `SampleHierarchy` (gpu_function, L111), `SegmentSignedDistance` (gpu_function, L61), `SplitDistance` (gpu_function, L73), `InitializeSplitDisplacement` (gpu_kernel, L1)
...
`ActiveSegment` (gpu_function, L72), `AssignFrontierLoops` (gpu_function, L469), `BuildFrontierEndpointHash` (gpu_function, L337), `BuildHalfEdgeArguments` (gpu_function, L114), `BuildHalfEdgeSpatialHash` (gpu_function, L180), `ClearHalfEdgeDerivedState` (gpu_function, L146), `ClearHalfEdgeHashState` (gpu_function, L128), `ContactViewState` (gpu_function, L100), `CreateFrontierLoops` (gpu_function, L447), `FilmPoint` (gpu_function, L67), `FinalizeFrontierLoops` (gpu_function, L506), `FindLoopRoot` (gpu_function, L106), `HashCell` (gpu_function, L89), `HookFrontierComponents` (gpu_function, L425), `InitializeFrontierComponents` (gpu_function, L415), `MaterializeMeasuredHalfEdges` (gpu_function, L161), `OrderOuterHalfEdges` (gpu_function, L355), `ProveHalfEdgeTwins` (gpu_function, L197), `SegmentDomain` (gpu_function, L62), `ShortcutFrontierComponents` (gpu_function, L439), `WorldCell` (gpu_function, L98), `AssignFrontierLoops` (gpu_kernel, L13), `BuildFrontierEndpointHash` (gpu_kernel, L7), `BuildHalfEdgeArguments` (gpu_kernel, L1), `BuildHalfEdgeSpatialHash` (gpu_kernel, L5), `ClearHalfEdgeDerivedState` (gpu_kernel, L3), `ClearHalfEdgeHashState` (gpu_kernel, L2), `CreateFrontierLoops` (gpu_kernel, L12), `FinalizeFrontierLoops` (gpu_kernel, L14), `HookFrontierComponents` (gpu_kernel, L10), `InitializeFrontierComponents` (gpu_kernel, L9), `MaterializeMeasuredHalfEdges` (gpu_kernel, L4), `OrderOuterHalfEdges` (gpu_kernel, L8), `ProveHalfEdgeTwins` (gpu_kernel, L6), `ShortcutFrontierComponents` (gpu_kernel, L11)

### `Runtime/Resources/Prism/MeshletBuild.compute`

`AddMeshletBuildGroupOffsets` (gpu_function, L1205), `BootstrapCoverage` (gpu_function, L509), `BootstrapCoverageCell` (gpu_function, L499), `BoundaryCurveCache` (struct, L117), `BoundaryHashEntry` (struct, L105), `BoundaryHashKey` (gpu_function, L570), `BoundarySignedDistance` (gpu_function, L676), `BuildAdaptiveFilmMeshlets` (gpu_function, L1419), `BuildFilmCount` (gpu_function, L276), `BuildFilmIndex` (gpu_function, L282), `BuildMeshDispatchArguments` (gpu_function, L945), `BuildSurfaceVertex` (gpu_function, L1335), `CachedBoundarySegment` (gpu_function, L581), `CachedBoundarySide` (gpu_function, L590), `CellTouchesPersistentBoundary` (gpu_function, L662), `ChooseSubdivision` (gpu_function, L873), `ClassifyMaterialTriangle` (gpu_function, L771), `ClearMeshletBuild` (gpu_function, L926), `ClipTriangle` (gpu_function, L1361), `ClosestBoundaryPoint` (gpu_function, L609), `ClosestPointOnUvSegment` (gpu_function, L599), `CommitDirtyMeshletBuild` (gpu_function, L1003), `CommitFullMeshletRepack` (gpu_function, L1271), `ContactBoundaryHeader` (struct, L83), `ContactFilmHeader` (struct, L17), `ContactMeshletDescriptor` (struct, L64), `ContactMeshletVertex` (struct, L47), `CountMeshletBuildPlans` (gpu_function, L1042), `DisplacementCell` (struct, L142), `DisplacementMetrics` (gpu_function, L351), `DisplacementPageHeader` (struct, L125), `ElasticChartState` (struct, L190), `EmitClippedPolygon` (gpu_function, L1388), `FilmMeshletRange` (struct, L176), `FilmSlotState` (struct, L185), `FinalizeDirtyMeshletFilms` (gpu_function, L1292), `FinalizeMeshletDrawArguments` (gpu_function, L1677), `Height` (gpu_function, L798), `LoadDisplacementCell` (gpu_function, L288), `LoadDisplacementChild` (gpu_function, L300), `MeshletBuildPlan` (struct, L156), `NonDegenerateCutScalar` (gpu_function, L750), `PrepareDirtyMeshletBuild` (gpu_function, L962), `PrepareFullMeshletRepack` (gpu_function, L1024), `RecoverPreviousMeshletGeneration` (gpu_function, L1650), `Reserve` (gpu_function, L853), `ReservedIndexCount` (gpu_function, L919), `ReservedSubdivision` (gpu_function, L905), `ReservedVertexCount` (gpu_function, L911), `ResolveDisplacementPage` (gpu_function, L310), `SampleDisplacement` (gpu_function, L403), `ScanMeshletBuildGroups` (gpu_function, L1183), `ScanMeshletBuildPlans` (gpu_function, L1147), `StableSurfaceSampleId` (gpu_function, L1324), `SurfaceCoverage` (gpu_function, L525), `SurfaceNormal` (gpu_function, L813), `SurfacePosition` (gpu_function, L844), `TriangleBoundaryCut` (gpu_function, L758), `TriangleMask` (gpu_function, L743), `ValidateDirtyMeshletBuild` (gpu_function, L976), `ValidateMeshletBuild` (gpu_function, L1220), `AddMeshletBuildGroupOffsets` (gpu_kernel, L6), `BuildAdaptiveFilmMeshlets` (gpu_kernel, L8), `BuildMeshDispatchArguments` (gpu_kernel, L2), `ClearMeshletBuild` (gpu_kernel, L1), `CommitDirtyMeshletBuild` (gpu_kernel, L14), `CommitFullMeshletRepack` (gpu_kernel, L16), `CountMeshletBuildPlans` (gpu_kernel, L3), `FinalizeDirtyMeshletFilms` (gpu_kernel, L13), `FinalizeMeshletDrawArguments` (gpu_kernel, L10), `PrepareDirtyMeshletBuild` (gpu_kernel, L11), `PrepareFullMeshletRepack` (gpu_kernel, L15), `RecoverPreviousMeshletGeneration` (gpu_kernel, L9), `ScanMeshletBuildGroups` (gpu_kernel, L5), `ScanMeshletBuildPlans` (gpu_kernel, L4), `ValidateDirtyMeshletBuild` (gpu_kernel, L12), `ValidateMeshletBuild` (gpu_kernel, L7)
...
`BootstrapCoverage` (gpu_function, L91), `BuildFilmDirtyImportArguments` (gpu_function, L193), `BuildFilmDirtyTailArguments` (gpu_function, L210), `BuildSupportContourArguments` (gpu_function, L229), `CaseEdges` (gpu_function, L138), `CaseSegmentCount` (gpu_function, L119), `CommitSupportContourPages` (gpu_function, L324), `ContourSlot` (gpu_function, L164), `CountSupportContourSegments` (gpu_function, L238), `Coverage` (gpu_function, L100), `DirtyCount` (gpu_function, L70), `DisplacementCell` (struct, L25), `DisplacementPageHeader` (struct, L17), `EdgePoint` (gpu_function, L125), `EnqueueTopologyFilm` (gpu_function, L72), `FinalizeTopologyDirtyFlags` (gpu_function, L468), `ImportFilmDirtyTopology` (gpu_function, L202), `ImportFilmDirtyTopologyTail` (gpu_function, L221), `InitializeSupportContourAtlas` (gpu_function, L178), `PreflightSupportContourPages` (gpu_function, L287), `QueueElasticTopologyChanges` (gpu_function, L485), `ResetRestoredTopologyTransient` (gpu_function, L500), `ResetTopologyDirtyQueue` (gpu_function, L478), `WriteSupportContourSegments` (gpu_function, L397), `if` (gpu_function, L348), `BuildFilmDirtyImportArguments` (gpu_kernel, L2), `BuildFilmDirtyTailArguments` (gpu_kernel, L4), `BuildSupportContourArguments` (gpu_kernel, L6), `CommitSupportContourPages` (gpu_kernel, L9), `CountSupportContourSegments` (gpu_kernel, L7), `FinalizeTopologyDirtyFlags` (gpu_kernel, L11), `ImportFilmDirtyTopology` (gpu_kernel, L3), `ImportFilmDirtyTopologyTail` (gpu_kernel, L5), `InitializeSupportContourAtlas` (gpu_kernel, L1), `PreflightSupportContourPages` (gpu_kernel, L8), `QueueElasticTopologyChanges` (gpu_kernel, L13), `ResetRestoredTopologyTransient` (gpu_kernel, L14), `ResetTopologyDirtyQueue` (gpu_kernel, L12), `WriteSupportContourSegments` (gpu_kernel, L10)
...
`BoundedVariance` (method, L669), `BuildEquations` (method, L498), `ClampMagnitude` (method, L688), `ContactMeshSnapshot` (class, L15), `Copy` (method, L340), `Equals` (method, L814), `Equals` (method, L819), `Estimate` (method, L380), `EstimateAsync` (method, L288), `EstimateAsync` (method, L370), `Failure` (method, L258), `FindNearest` (method, L764), `Finite` (method, L145), `Finite` (method, L150), `Finite` (method, L222), `Finite` (method, L719), `Finite` (method, L73), `Finite` (method, L78), `FinitePose` (method, L210), `GetHashCode` (method, L824), `HasEnoughOverlap` (method, L480), `IOverlapConstraintEstimator` (interface, L280), `Key` (method, L793), `NoneOverlapConstraintEstimator` (class, L285), `NormalAt` (method, L39), `NormalEquations` (class, L736), `Normalize` (method, L708), `OverlapConstraintEstimate` (class, L228), `OverlapPointCloud` (class, L25), `OverlapPointCloudBuilder` (class, L89), `OverlapRegistrationRequest` (class, L161), `PointAt` (method, L38), `PointToPlaneIcpEstimator` (class, L358), `PointToPlaneIcpSettings` (class, L297), `QuaternionFromRotationVector` (method, L696), `Range` (method, L345), `Sample` (struct, L724), `SelectSamples` (method, L552), `SourceSpatialIndex` (class, L744), `Success` (method, L265), `SwapRows` (method, L676), `TryCovariance` (method, L616), `TryCreate` (method, L186), `TryCreate` (method, L40), `TryCreate` (method, L91), `TrySolve` (method, L565), `TryValidate` (method, L314), `VoxelKey` (struct, L801)
...
`AssertKernels` (method, L254), `AtlasWorkGraphKernelsImport` (method, L66), `BoundaryCurveIsSharedAndCachedBeforeMaterialization` (method, L181), `CanonicalAtlasAbiMatchesDeclaredStrides` (method, L18), `CanonicalSplitFollowsEvidenceAndCreatesTwoHypotheses` (method, L194), `ChunkStagePreservesGlobalManifoldAndPortalTopology` (method, L209), `ContinuationsRequireIndependentPhysicalEvidence` (method, L169), `DefaultAtlasBindingsFitQuestVulkanStorageLimit` (method, L40), `KeyframesAreSelectedByGpuInformationGain` (method, L223), `MeasuredSupportCreatesContoursAndLatentStateNeverPredicts` (method, L152), `NextPowerOfTwo` (method, L270), `PressureDiagnosticAbiIsStable` (method, L238), `PrismTopologyContractTests` (class, L11), `RectangleFrontierOntologyIsAbsent` (method, L104), `RuntimeRoot` (method, L267), `Source` (method, L264), `SpawnPublishesOneCoordinateConsistentComponentPosterior` (method, L130)
