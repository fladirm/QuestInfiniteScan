...
Source digest: `442932a1331fba96c0e54ef5abd8e544ebe1f99976d6170798ba214f4420e6ea`

Scope: 91 files, 1349 symbols, 1031 functions/methods, 69 GPU kernels, 24 event links.
...
- `RoomScanPersistence.LoadCompleted` → `ApplyRenderMode` ([Runtime/Core/RoomScanner.cs:373](../../Runtime/Core/RoomScanner.cs#L373))
- `RoomAnchorManager.RoomReady` → `OnRoomAnchorReady` ([Runtime/Core/RoomScanner.cs:382](../../Runtime/Core/RoomScanner.cs#L382))
- `TextureRefinement.StatusChanged` → `statusHandler` ([Runtime/Core/RoomScanner.cs:1123](../../Runtime/Core/RoomScanner.cs#L1123))
- `TextureRefinement.StatusChanged` → `hqStatusHandler` ([Runtime/Core/RoomScanner.cs:1259](../../Runtime/Core/RoomScanner.cs#L1259))
- `RoomUnderstanding.AnchorsChanged` → `OnMrukAnchorsChanged` ([Runtime/Core/RoomScanner.cs:1579](../../Runtime/Core/RoomScanner.cs#L1579))
...
- `PrismFilmUpdater.UpdateCompleted` → `OnFilmUpdateCompleted` ([Runtime/Prism/Geometry/PrismBoundaryGraph.cs:136](../../Runtime/Prism/Geometry/PrismBoundaryGraph.cs#L136))
...
- `RenderPipelineManager.beginCameraRendering` → `OnBeginCameraRendering` ([Runtime/Prism/Geometry/PrismWorldMeshletRenderer.cs:46](../../Runtime/Prism/Geometry/PrismWorldMeshletRenderer.cs#L46))
...
- `SubmapManager.RolloverRequested` → `OnRolloverRequested` ([Runtime/World/PrismChunkResidencyManager.cs:55](../../Runtime/World/PrismChunkResidencyManager.cs#L55))
- `SubmapManager.ActiveChunkChanged` → `OnActiveChunkChanged` ([Runtime/World/PrismChunkResidencyManager.cs:56](../../Runtime/World/PrismChunkResidencyManager.cs#L56))
- `SubmapManager.ChunkRevisionPublished` → `OnLegacyRevisionPublished` ([Runtime/World/PrismChunkResidencyManager.cs:57](../../Runtime/World/PrismChunkResidencyManager.cs#L57))
- `SubmapManager.PoseGraphRefined` → `OnPoseGraphRefined` ([Runtime/World/PrismChunkResidencyManager.cs:58](../../Runtime/World/PrismChunkResidencyManager.cs#L58))
- `RoomScanner.ScanStarted` → `OnScanStarted` ([Runtime/World/PrismChunkResidencyManager.cs:59](../../Runtime/World/PrismChunkResidencyManager.cs#L59))
...
`AddExclusionZone` (method, L1088), `ApplyEnhancedMesh` (method, L1468), `ApplyHQTexture` (method, L1595), `ApplyRefinedAtlas` (method, L1489), `ApplyRefinedTexture` (method, L1537), `ApplyRenderMode` (method, L1950), `Awake` (method, L347), `BuildCoverage` (method, L1787), `BuildProgress` (method, L1813), `CacheComponents` (method, L396), `ClearAllDataAsync` (method, L873), `ClearScan` (method, L856), `CompleteRoomStartup` (method, L449), `ConfigurePrismChunk` (method, L765), `CreateScanAnchorAsync` (method, L722), `CycleRenderMode` (method, L943), `DeleteActiveArtifact` (method, L2033), `DeleteEnhancedMesh` (method, L2075), `DeserializeRefinedMesh` (method, L1434), `EnsureRefinedMesh` (method, L1676), `EnsureRefinedRenderer` (method, L1693), `EnsureUnwrappedAsync` (method, L1607), `ExportPointCloudAsync` (method, L1060), `FreezeInView` (method, L994), `GetActiveCameraProvider` (method, L2152), `IsModeAvailable` (method, L975), `LoadDownloadedSplat` (method, L2107), `LoadPackageAsync` (method, L1040), `LoadRefinedOnlyAsync` (method, L1051), `OnDisable` (method, L456), `OnMrukAnchorsChanged` (method, L1589), `OnRoomAnchorReady` (method, L387), `PausePrismForResidency` (method, L759), `PopulateSceneObjectRegistry` (method, L1566), `ProvideColorFrame` (method, L1884), `ReconstructUnwrapFromResult` (method, L1652), `ReleaseScanResources` (method, L833), `RemoveExclusionZone` (method, L1097), `ResumePrismAfterResidency` (method, L760), `RoomScanner` (class, L94), `RunServerTrainingAsync` (method, L2118), `SaveScanAsync` (method, L1021), `ScanCoverage` (struct, L52), `ScanPhase` (enum, L37), `ScanProgress` (struct, L77), `ScanRenderMode` (enum, L16), `SerializeRefinedMesh` (method, L1407), `SetCameraProvider` (method, L1080), `SetRefinedWorldFromLocal` (method, L1719), `SetRenderMode` (method, L931), `SetSafeShaderDefaults` (method, L1872), `SetSceneObjectRegistry` (method, L1556), `SetupHeadExclusion` (method, L1767), `Start` (method, L355), `StartHQRefinement` (method, L1235), `StartMeshEnhancement` (method, L1344), `StartPrismPipeline` (method, L777), `StartScanningAsync` (method, L562), `StartServerTraining` (method, L1065), `StartTextureRefinement` (method, L1111), `StopPrismPipeline` (method, L796), `StopScanning` (method, L740), `SubscribeToAnchorsChanged` (method, L1575), `ToggleDebugMenu` (method, L1072), `ToggleScanning` (method, L818), `TryGetCameraIntrinsics` (method, L1731), `UnfreezeInView` (method, L1007), `UnsubscribeFromAnchorsChanged` (method, L1582), `Update` (method, L466), `UpdatePlateauDetection` (method, L1839)
...
`AllocateFrameBuffers` (method, L272), `BindFrame` (method, L226), `BindPersistent` (method, L288), `CeilDiv` (method, L340), `ConfigureFrame` (method, L210), `DisposeFrameBuffers` (method, L319), `Intrinsics` (method, L333), `OnDestroy` (method, L171), `OnFilmUpdateCompleted` (method, L180), `PoseMatrix` (method, L337), `PrismBoundaryGraph` (class, L15), `RebuildCanonicalIndex` (method, L141), `SetChunkFrame` (method, L97), `StartTracking` (method, L100), `StopTracking` (method, L164)
...
`Draw` (method, L116), `LateUpdate` (method, L80), `OnBeginCameraRendering` (method, L94), `OnDestroy` (method, L143), `OnDisable` (method, L48), `OnEnable` (method, L35), `PrismWorldMeshletRenderer` (class, L15), `RegisterResident` (method, L57), `RemoveResident` (method, L67), `Retire` (method, L137), `SetActive` (method, L51), `SetResidentTransform` (method, L73)
...
`AccumulateBoundaryAbsence` (gpu_function, L661), `AccumulateBoundaryEvents` (gpu_function, L558), `AtomicAddScaled` (gpu_function, L250), `BoundaryHashEntry` (struct, L81), `BuildBoundarySolveArguments` (gpu_function, L698), `ClearBoundaryFrame` (gpu_function, L550), `ClearLoadedBoundaryHash` (gpu_function, L511), `ConeEvent` (struct, L9), `ContactBoundaryHeader` (struct, L59), `ContactFilmHeader` (struct, L31), `Cubic` (gpu_function, L225), `DistanceToCurve` (gpu_function, L232), `FindAdjacentFilm` (gpu_function, L465), `FindBoundary` (gpu_function, L271), `HashKey` (gpu_function, L143), `Height` (gpu_function, L190), `InitializeBoundaryState` (gpu_function, L489), `LoadDepthRay` (gpu_function, L154), `LoadRgb` (gpu_function, L160), `Luminance` (gpu_function, L167), `MarkDirty` (gpu_function, L258), `PointToUv` (gpu_function, L218), `RefineUvWithRgb` (gpu_function, L390), `RehashLoadedBoundaries` (gpu_function, L517), `ResolveBoundary` (gpu_function, L293), `SobelMagnitude` (gpu_function, L172), `SolveDirtyBoundaries` (gpu_function, L705), `SurfaceNormal` (gpu_function, L205), `SurfacePoint` (gpu_function, L197), `ViewBin` (gpu_function, L379), `AccumulateBoundaryAbsence` (gpu_kernel, L6), `AccumulateBoundaryEvents` (gpu_kernel, L5), `BuildBoundarySolveArguments` (gpu_kernel, L7), `ClearBoundaryFrame` (gpu_kernel, L4), `ClearLoadedBoundaryHash` (gpu_kernel, L2), `InitializeBoundaryState` (gpu_kernel, L1), `RehashLoadedBoundaries` (gpu_kernel, L3), `SolveDirtyBoundaries` (gpu_kernel, L8)
...
`ActivateAfterAsync` (method, L162), `AwaitGpuIdleAsync` (method, L330), `BeginStage` (method, L113), `Bind` (method, L41), `LoadSnapshot` (method, L317), `OnActiveChunkChanged` (method, L151), `OnDestroy` (method, L65), `OnLegacyRevisionPublished` (method, L245), `OnPoseGraphRefined` (method, L88), `OnRolloverRequested` (method, L82), `OnScanStarted` (method, L107), `OnScanStopped` (method, L101), `PrismChunkResidencyManager` (class, L20), `ResolveSnapshotAsync` (method, L224), `RetainRecent` (method, L276), `RetainWhenReadyAsync` (method, L134), `Start` (method, L39), `TouchRecent` (method, L310)
...
`ApplyLargeWorldDefaults` (method, L128), `Awake` (method, L238), `BeginRestoreChunkKeyframes` (method, L1089), `CancelPendingRollover` (method, L500), `ChunkRefinementContext` (class, L14), `ConfigureChunkKeyframes` (method, L1036), `EmitWorldProfile` (method, L556), `FinalizeActiveChunkAsync` (method, L707), `FinalizeAndAdvanceAsync` (method, L627), `FindChunk` (method, L1029), `GetChunkKeyframeDirectory` (method, L1143), `GetChunkPublicationTail` (method, L1018), `IsCompatibleVolumeSnapshot` (method, L1228), `KeyframeRestoreResult` (class, L1272), `LoadVolumeArtifact` (method, L1155), `OnDestroy` (method, L275), `OnModuleInitialize` (method, L244), `OnRenderModeChanged` (method, L1083), `OnScanAnchorCreated` (method, L1047), `OnScanStarted` (method, L290), `OnScanStopped` (method, L317), `OverlapObservation` (class, L1249), `PersistFinalizedChunkAfterAsync` (method, L960), `PoseApproximately` (method, L1222), `PublicationKey` (method, L1024), `PublishRefinedArtifactsAsync` (method, L189), `QueueBackgroundPublication` (method, L941), `QueueOverlapRefinement` (method, L804), `RefineOverlapAfterAsync` (method, L821), `RefreshRuntimeGraphFrames` (method, L910), `ResetOverlapPipeline` (method, L925), `RestoreActiveVolumeAsync` (method, L579), `RestoreChunkKeyframesAsync` (method, L1118), `RestoreKeyframeArtifact` (method, L1179), `SetFinalizationStatus` (method, L1149), `SetPoseGraphStatus` (method, L936), `SubmapManager` (class, L30), `TryAttachWorld` (method, L381), `TryCaptureRefinementContext` (method, L148), `TryCompletePendingRollover` (method, L431), `TryCompletePendingRollover` (method, L436), `TryCompletePreparedRollover` (method, L754), `TryStartNewWorld` (method, L326), `Update` (method, L512), `VolumeArtifactLoadResult` (class, L1266), `WaitForRefinementReadyAsync` (method, L134)
