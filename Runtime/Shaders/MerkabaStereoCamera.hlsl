    ...
    float2 currentResolution, out int2 texel, out float2 fraction)
{
    texel = 0;
    fraction = 0.0.xx;
    if (!all(isfinite(worldPosition)) || !all(isfinite(cameraPosition)) ||
        !all(isfinite(focalLength)) || !all(isfinite(principalPoint)) ||
        !all(isfinite(sensorResolution)) || !all(isfinite(currentResolution)) ||
    ...
    if (!all(isfinite(local)) || !(local.z > 0.0)) return false;
    ...
    pixel = pixel - 0.5;
    if (!all(isfinite(pixel)) || any(pixel < 0.0) ||
...
bool MerkabaTrySampleCameraRgb(uint eye, float3 worldPosition, out float3 rgb)
{
    ...
    [unroll] for (uint tap = 0u; tap < 4u; ++tap)
    {
        ...
        if (!all(isfinite(captured)) || any(captured < 0.0)) return false;
