...
uint M8FlowerResolveHaloRef(int3 tile)
{
    ...
    if (chunkRef - 1u >= MERKABA_M8_CHUNK_CAPACITY)
        return M8_FLOWER_HALO_INVALID << 30u;
    uint tileRef = _M8ChunkTileRefsRead[(chunkRef - 1u) *
...
void M8FlowerCacheTileHalo(uint ownerSlot, uint lane)
{
    if (lane == 0u)
        m8FlowerHaloTile = M8GlobalKernelCoordRead(ownerSlot, 0u) >> 3;
    GroupMemoryBarrierWithGroupSync();
    if (lane < M8_FLOWER_HALO_COUNT)
    {
        uint packed = M8FlowerResolveHaloRef(
            m8FlowerHaloTile + M8FlowerHaloDelta(lane));
        m8FlowerHalo[lane] = packed;
        _M8TileHalo[ownerSlot * M8_FLOWER_HALO_COUNT + lane] = packed;
    }
    GroupMemoryBarrierWithGroupSync();
}
    ...
    out uint kernelLocal)
{
    ...
    if (M8LoadTileRuntimeRead(slot).w != generation) return false;
    ...
    if (any((M8GlobalKernelCoordRead(slot, 0u) >> 3) !=
