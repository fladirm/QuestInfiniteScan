...
namespace Genesis.RoomScan
{
    public static partial class MerkabaSphereFlowerAuthority
    {
        ...
        private static bool TryBinary32Units(float value, out BigInteger units)
        {
            units = BigInteger.Zero;
            uint bits = math.asuint(value);
            uint exponent = (bits >> 23) & 255u;
            if (exponent == 255u) return false;
            uint mantissa = (bits & 0x7fffffu) | (exponent == 0u ? 0u : 0x800000u);
            units = (BigInteger)mantissa << (exponent == 0u ? 0 : (int)exponent - 1);
            if ((bits & 0x80000000u) != 0u) units = -units;
            return true;
        }
        ...
        private static bool BoundaryLinearSign(float4 value, int4 coefficient, out int sign)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                if (!TryBinary32Units(value[i], out BigInteger units) || coefficient[i] <= -(1 << 24) ||
                    coefficient[i] >= (1 << 24)) return false;
                sum += units * coefficient[i];
            }
            sign = sum.Sign;
            return true;
        }
