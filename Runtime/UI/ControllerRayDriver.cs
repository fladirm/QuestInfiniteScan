...
namespace Genesis.RoomScan.UI
{
    ...
    public sealed class ControllerRayDriver : MonoBehaviour
    {
        ...
        {
            if (_hasTrackedPose && _line != null) DrawLaser();
        }
        ...
        {
            // Input consumers run after this Update. LateUpdate draws the
            // same hit; it must not decide ownership a frame after painting.
            _hoveringUi = Physics.Raycast(_rayHelper.position,
                    _rayHelper.forward, out RaycastHit hit,
