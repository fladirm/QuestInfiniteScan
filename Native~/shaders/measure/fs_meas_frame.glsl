...
// Twin: fs::meas::PushCompact (48 B). zp = (nearZ, farZ, maxDepthM, pad); a = (layers, budget, maxOut, flags); b = (obsId, frame, width, height).
...
int fsMeasClassify(uint layer, uint x, uint y, out FsTexelGeometry g) {
    ...
    if (!fsMeasDepthUsable(g.z, nearZ, maxDepth)) return 0;
    ...
    if (!(fsMeasDepthUsable(g.zxm, nearZ, maxDepth) && fsMeasDepthUsable(g.zxp, nearZ, maxDepth) &&
          fsMeasDepthUsable(g.zym, nearZ, maxDepth) && fsMeasDepthUsable(g.zyp, nearZ, maxDepth))) return 0;
