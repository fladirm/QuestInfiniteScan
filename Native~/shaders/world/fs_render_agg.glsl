...
void fsAggLeaf(uint base, uint n, uint blockId, inout FsRenderNode r) {
    ...
    uvec3 col = uvec3(clamp(colsum * inv + vec3(0.5), vec3(0.0), vec3(255.0)));
    r.node.repColorOrHandle = col.x | (col.y << 8) | (col.z << 16) | 0xFF000000u;
...
void fsAggInternal(inout FsRenderNode r) {
    ...
    float areaSum = 0.0, wsum = 0.0; uint count = 0u, nchild = 0u; bool anyDir = false;
