...
namespace Genesis.RoomScan.Editor
{
    ...
    public static partial class MerkabaSphereFlowerCodegen
    {
        ...
        public const string GeneratedTableBlobAssetPath =
            "Packages/com.genesis.roomscan/Runtime/Merkaba/MerkabaFlowerTableBlob.generated.cs";
        ...
        public const string GeneratedDepthCertificateAssetPath =
            "Packages/com.genesis.roomscan/Tests/Editor/MerkabaDepthCertificate.generated.hlsl";
        public const string GeneratedObservationProjectionAssetPath =
            "Packages/com.genesis.roomscan/Runtime/Shaders/MerkabaObservationProjection.generated.hlsl";
        ...
        public static void Regenerate()
        {
            ...
            string observationProjection = BuildObservationProjectionHlsl();
            string tableBlob = BuildGeneratedTableBlob();
            ...
            WriteIfChanged(GeneratedObservationProjectionAssetPath, observationProjection);
            WriteIfChanged(GeneratedTableBlobAssetPath, tableBlob);
        ...
        public static void CheckForBatch()
        {
            ...
            CheckExact(GeneratedObservationProjectionAssetPath, BuildObservationProjectionHlsl());
            CheckExact(GeneratedTableBlobAssetPath, BuildGeneratedTableBlob());
        }
        ...
        internal static string BuildGeneratedTableBlob()
        {
            // The same emission pass supplies the accessors and every word.
            // There is no independently maintained GPU copy of CPU incidence.
            BuildGeneratedHlsl(out GpuTables tables);
            ...
            output.AppendLine("// Exact uint4 rows of the shared read-only GPU tables; no metric recomputation.");
            output.AppendLine("using Unity.Mathematics;");
            output.AppendLine("namespace Genesis.RoomScan");
            output.AppendLine("{");
            output.AppendLine("    internal static class MerkabaFlowerTableBlob");
            output.AppendLine("    {");
            output.Append("        internal const int ByteSize = ").Append(checked(16 * rowCount)).AppendLine(";");
            output.Append("        internal const int RowCount = ").Append(rowCount).AppendLine(";");
            output.Append("        internal const uint TableHash = 0x")
                .Append(MerkabaSphereFlowerAuthority.ComputeFrozenTableHash().ToString("x8", CultureInfo.InvariantCulture)).AppendLine("u;");
        ...
        // Codegen-only word sink. Scalars share uint4 rows; vectors retain one
        // aligned row each. Runtime shaders see only this immutable SRV.
        private sealed class GpuTables
        {
            internal readonly List<uint4> Rows = new();
            internal readonly List<(string Name, int Offset)> Sections = new();

            internal GpuTables()
            {
                var sectors = MerkabaSphereFlowerAuthority.SectorBoundaries;
                var rules = MerkabaSphereFlowerAuthority.BoundaryRules;
                if (sectors.Length != rules.Length)
                    throw new InvalidOperationException("Sector/boundary table lengths differ.");
                Sections.Add(("SectorBounds", Rows.Count));
                foreach (var sector in sectors)
                    Rows.Add(math.asuint(new float4(sector.Enclosure.X.Lower,
                        sector.Enclosure.X.Upper, sector.Enclosure.Y.Lower,
                        sector.Enclosure.Y.Upper)));
                Sections.Add(("BoundaryRational", Rows.Count));
                foreach (var rule in rules) Rows.Add(math.asuint(rule.Rational));
                Sections.Add(("BoundaryIrrationalOwner", Rows.Count));
                foreach (var rule in rules)
                    Rows.Add(math.asuint(new int4(rule.D, rule.Owner)));
                Sections.Add(("L2CanonicalOwner", Rows.Count));
                Rows.AddRange(BuildL2CanonicalOwnerRows());
                Sections.Add(("L2Carrier", Rows.Count));
                Rows.AddRange(BuildL2CarrierRows());
            }

            internal void AppendVectors(StringBuilder output, string name,
                string type, ReadOnlySpan<uint4> rows)
            {
                int first = Rows.Count;
                Sections.Add((name.Substring("M8Flower".Length), first));
                foreach (uint4 row in rows) Rows.Add(row);
                string expression = "_M8FlowerTables[" + first + "u+index]";
                if (type.StartsWith("float", StringComparison.Ordinal))
                    ...
                    expression = "asint(" + expression + ")";
                if (type.EndsWith("2", StringComparison.Ordinal)) expression += ".xy";
                if (type.EndsWith("3", StringComparison.Ordinal)) expression += ".xyz";
                output.Append(type).Append(' ').Append(name)
                    .Append("At(uint index) { return ").Append(expression).AppendLine("; }");
            }

            internal void AppendScalars(StringBuilder output, string name,
                string type, ReadOnlySpan<uint> words)
            {
                int first = Rows.Count;
                Sections.Add((name.Substring("M8Flower".Length), first));
                for (int item = 0; item < words.Length; item += 4)
                {
                    uint4 row = 0u;
                    for (int lane = 0; lane < 4 && item + lane < words.Length; lane++)
                        row[lane] = words[item + lane];
                    Rows.Add(row);
                }
                string expression = "_M8FlowerTables[" + first + "u+(index>>2u)][index&3u]";
                if (type == "float") expression = "asfloat(" + expression + ")";
                if (type == "int") expression = "asint(" + expression + ")";
                output.Append(type).Append(' ').Append(name)
                    .Append("At(uint index) { return ").Append(expression).AppendLine("; }");
            }
        }

        // Match Number(float)'s pre-existing zero spelling exactly. Nonzero
        // values were round-trip FP32 literals and are copied bit for bit.
        private static uint TableFloatBits(float value) => value == 0f ? 0u : math.asuint(value);
        private static uint4 TableFloatBits(float3 value) => new uint4(
            TableFloatBits(value.x), TableFloatBits(value.y), TableFloatBits(value.z), 0u);
        ...
        private static uint4[] BuildL2CanonicalOwnerRows()
        {
            ...
            int count = MerkabaSphereFlowerAuthority.L2WedgeCount;
            if (offsets.Length != count + 1 || offsets[count] != owners.Length)
                throw new InvalidOperationException("Incomplete L2 canonical-owner source.");
            var rows = new uint4[count];
            for (int wedge = 0; wedge < count; wedge++)
            {
                int first = offsets[wedge], end = offsets[wedge + 1];
                if (first >= end || end > owners.Length)
                    throw new InvalidOperationException("L2 wedge has no canonical owner.");
                var row = owners[first];
                if (!MerkabaSphereFlowerAuthority.TryL2CanonicalWedgeOwner(int3.zero,
                        wedge / 6, wedge % 6, out int3 delta, out int partner, out byte map) ||
                    math.any(delta != row.OwnerOffset) || partner != row.PartnerWedge || map != row.VertexMap)
                    throw new InvalidOperationException("L2 canonical-owner view differs from its exact source.");
                rows[wedge] = math.asuint(new int4(row.OwnerOffset, unchecked((int)row.Packed)));
            }
            return rows;
        }

        private static uint4[] BuildL2CarrierRows()
        {
            var wedges = MerkabaSphereFlowerAuthority.L2Wedges;
            int count = MerkabaSphereFlowerAuthority.L2HubCount;
            if (wedges.Length != count * 6)
                throw new InvalidOperationException("Incomplete L2 carrier incidence.");
            var rows = new uint4[count];
            var sites = new int[7];
            for (int carrier = 0; carrier < count; carrier++)
            {
                uint4 row = 0u;
                for (int site = 0; site < 7; site++)
                {
                    int knot = MerkabaSphereFlowerAuthority.L2CarrierKnotIndex(carrier, site);
                    if ((uint)knot >= MerkabaSphereFlowerAuthority.L2KnotCount)
                        throw new InvalidOperationException("L2 carrier references an invalid knot.");
                    sites[site] = knot;
                    row[site >> 1] |= (uint)knot << (16 * (site & 1));
                }
                uint owned = 0u;
                for (int wedge = 0; wedge < 6; wedge++)
                {
                    var source = wedges[6 * carrier + wedge];
                    if (sites[0] != source.Hub || sites[1 + wedge] != source.Ring0 ||
                        sites[1 + (wedge + 1) % 6] != source.Ring1)
                        throw new InvalidOperationException("Seven-site view changes original L2 wedge incidence.");
                    if (MerkabaSphereFlowerAuthority.L2OwnsWedge(carrier, wedge)) owned |= 1u << wedge;
                }
                uint skinSource = wedges[6 * carrier].Source;
                if (skinSource >= 1024u)
                    throw new InvalidOperationException("Canonical L2 source exceeds its packed field.");
                // Four exact pairs of uint16: the seventh knot occupies w.lo.
                // w.hi carries owned-wedge bits6 and original canonical source10.
                row.w |= (owned << 16) | (skinSource << 22);
                rows[carrier] = row;
            }
            return rows;
        }

        internal static string BuildGeneratedHlsl() => BuildGeneratedHlsl(out _);

        private static string BuildGeneratedHlsl(out GpuTables tables)
        {
            tables = new GpuTables();
            var output = new StringBuilder(48 * 1024);
            output.AppendLine("// GENERATED by MerkabaSphereFlowerCodegen. DO NOT EDIT.");
            ...
            output.AppendLine("#if defined(M8_FLOWER_SRV)");
            output.AppendLine("StructuredBuffer<uint4> _M8FlowerTables M8_FLOWER_SRV(t13);");
            output.AppendLine("#else");
            output.AppendLine("StructuredBuffer<uint4> _M8FlowerTables;");
            ...
            Define(output, "M8_FLOWER_L2_CANONICAL_OWNER_ROW_BASE",
                checked(3 * MerkabaSphereFlowerAuthority.SectorBoundaries.Length) + "u");
            Define(output, "M8_FLOWER_L2_CARRIER_ROW_BASE",
                checked(3 * MerkabaSphereFlowerAuthority.SectorBoundaries.Length +
                    MerkabaSphereFlowerAuthority.L2WedgeCount) + "u");
            ...
            AppendLines(output, tables);
            AppendDirections(output, tables);
            AppendNodes(output, tables);
            AppendStrands(output, tables);
            AppendPetals(output, tables);
            AppendCompletion(output, false, tables);
            AppendChildren(output, tables);
            AppendPhaseFamilies(output, tables);
            AppendL2Carrier(output, false, tables);
            AppendExcavation(output, tables);
            AppendSkin(output, tables);
            AppendSectors(output, tables);
            AppendBoundaryRules(output, false);
            AppendRadicalSectorPatterns(output, false, tables);
...
");
        ...
        // Native FP supplies only a normal-range estimate. Exact integer
        // remainders recover the same bracket/RNE result on CPU and GPU.
        // No bit-at-a-time arithmetic in the production instruction stream.
...
";
            output.AppendLine(csharp
                ? source.Replace("precise ", string.Empty).Replace("[loop] ", string.Empty)
                    .Replace("[unroll] ", string.Empty)
                    .Replace("firstbithigh(mantissa)", "(31-lzcnt(mantissa))")
                : source.Replace("internal static ", string.Empty));
        }
...
bool4 M8FlowerIsFinite(float4 value) { return (asuint(value) & 0x7f800000u) != 0x7f800000u; }");
            ...
            o.AppendLine("struct M8ObservationRecord { uint TileAndKernel; uint SourcePixel; uint MeasuredPlane; uint Reserved; };");
            o.AppendLine(@"
    ...
    uint status, uint scratch)
{
    return level | (lineClass << M8_FLOWER_SYMBOL_LINE_SHIFT) |
        ...
        private static void AppendLines(StringBuilder output, GpuTables tables)
        {
            var lines = MerkabaSphereFlowerAuthority.Lines;
            var rows = new uint4[lines.Length];
            for (int i = 0; i < lines.Length; i++)
            {
                var line = lines[i];
                rows[i] = math.asuint(new int4(line.Direction, 0));
            }
            tables.AppendVectors(output, "M8FlowerLineDirection", "int3", rows);
            for (int i = 0; i < lines.Length; i++) rows[i] = TableFloatBits(lines[i].UnitDirection);
            tables.AppendVectors(output, "M8FlowerLineUnit", "float3", rows);
            for (int i = 0; i < lines.Length; i++) rows[i] = TableFloatBits(lines[i].E1);
            tables.AppendVectors(output, "M8FlowerLineE1", "float3", rows);
            for (int i = 0; i < lines.Length; i++) rows[i] = TableFloatBits(lines[i].E2);
            tables.AppendVectors(output, "M8FlowerLineE2", "float3", rows);
            for (int i = 0; i < lines.Length; i++)
            {
                var line = lines[i];
                rows[i] = new uint4((uint)line.Shell, line.SectorOffset, line.SectorCount, 0u);
            }
            tables.AppendVectors(output, "M8FlowerLineMeta", "uint3", rows);
        }

        private static void AppendDirections(StringBuilder output, GpuTables tables)
        {
            var rows = new uint4[MerkabaSphereFlowerAuthority.Directions.Length];
            for (int i = 0; i < MerkabaSphereFlowerAuthority.Directions.Length; i++)
            {
                var value = MerkabaSphereFlowerAuthority.Directions[i];
                rows[i] = math.asuint(new int4(value.Direction, value.LineClass));
            }
            tables.AppendVectors(output, "M8FlowerDirection", "int4", rows);
            for (int i = 0; i < MerkabaSphereFlowerAuthority.Directions.Length; i++)
            {
                var value = MerkabaSphereFlowerAuthority.Directions[i];
                rows[i] = math.asuint(new int4(value.Orientation, (int)value.Shell, 0, 0));
            }
            tables.AppendVectors(output, "M8FlowerDirectionMeta", "int2", rows);
        }

        private static void AppendNodes(StringBuilder output, GpuTables tables)
        {
            var rows = new uint4[MerkabaSphereFlowerAuthority.NodeIncidentPetals.Length];
            for (int i = 0; i < MerkabaSphereFlowerAuthority.NodeIncidentPetals.Length; i++)
            {
                ...
                rows[i] = new uint4((uint)mask, (uint)(mask >> 32), 0u, 0u);
            }
            tables.AppendVectors(output, "M8FlowerNodeIncidentPetals", "uint2", rows);
            rows = new uint4[MerkabaSphereFlowerAuthority.Nodes.Length];
            for (int i = 0; i < MerkabaSphereFlowerAuthority.Nodes.Length; i++)
            {
                var value = MerkabaSphereFlowerAuthority.Nodes[i];
                rows[i] = math.asuint(new int4(value.Direction, value.LineClass));
            }
            tables.AppendVectors(output, "M8FlowerNode", "int4", rows);
        }

        private static void AppendStrands(StringBuilder output, GpuTables tables)
        {
            var rows = new uint4[MerkabaSphereFlowerAuthority.Strands.Length];
            for (int i = 0; i < MerkabaSphereFlowerAuthority.Strands.Length; i++)
            {
                var value = MerkabaSphereFlowerAuthority.Strands[i];
                uint petals = (uint)(value.Petal0 | (value.Petal1 << 8));
                rows[i] = new uint4(value.Node0, value.Node1, value.IncidenceKind, petals);
            }
            tables.AppendVectors(output, "M8FlowerStrand", "uint4", rows);
        }

        private static void AppendPetals(StringBuilder output, GpuTables tables)
        {
            var rows = new uint4[MerkabaSphereFlowerAuthority.Petals.Length];
            for (int i = 0; i < MerkabaSphereFlowerAuthority.Petals.Length; i++)
            {
                var p = MerkabaSphereFlowerAuthority.Petals[i];
                rows[i] = new uint4(p.FaceNode, p.EdgeNode, p.CornerNode, p.Orientation > 0 ? 1u : 0u);
            }
            tables.AppendVectors(output, "M8FlowerPetalNodes", "uint4", rows);
            for (int i = 0; i < MerkabaSphereFlowerAuthority.Petals.Length; i++)
            {
                var p = MerkabaSphereFlowerAuthority.Petals[i];
                rows[i] = new uint4(p.Strand0, p.Strand1, p.Strand2, 0u);
            }
            tables.AppendVectors(output, "M8FlowerPetalStrands", "uint3", rows);
            for (int i = 0; i < MerkabaSphereFlowerAuthority.Petals.Length; i++)
            {
                var p = MerkabaSphereFlowerAuthority.Petals[i];
                rows[i] = math.asuint(new int4(p.StrandSign0, p.StrandSign1, p.StrandSign2, 0));
            }
            tables.AppendVectors(output, "M8FlowerPetalStrandSign", "int3", rows);
        }

        private static void AppendCompletion(StringBuilder output, bool csharp, GpuTables tables = null)
        {
            if (csharp) output.AppendLine("        private static readonly uint2[] M8FlowerCompletionNeighbours = new uint2[] {");
            var petals = MerkabaSphereFlowerAuthority.Petals;
            var strands = MerkabaSphereFlowerAuthority.Strands;
            var rows = new uint4[petals.Length];
            for (int petal = 0; petal < petals.Length; petal++)
            {
                ...
                rows[petal] = new uint4((uint)neighbours, (uint)(neighbours >> 32), 0u, 0u);
                if (csharp)
                {
                    output.Append("            new uint2(")
                        .Append((uint)neighbours).Append("u, ").Append((uint)(neighbours >> 32)).Append("u)");
                    Comma(output, petal, petals.Length);
                }
            }
            if (csharp) output.AppendLine("};");
            else tables.AppendVectors(output, "M8FlowerCompletionNeighbours", "uint2", rows);
            string source = @"
...
";
        ...
        private static void AppendLoopMetricLengths(StringBuilder output, GpuTables tables)
        {
            ...
            // Sensor-loop radii are finite lattice data too. Preserve the
            // exact former ordered enclosure, once in the Editor, not once
            // per pixel/hypothesis/loop on the headset.
            var radii = new uint4[MerkabaSphereFlowerAuthority.LineClassCount];
            var step = MerkabaSphereFlowerAuthority.FloatInterval.Singleton(
                (float)MerkabaSphereFlowerAuthority.LatticeStep);
            for (int line = 0; line < radii.Length; ++line)
            {
                var direction = MerkabaSphereFlowerAuthority.Lines[line].Direction;
                var factor = MerkabaSphereFlowerAuthority.FloatInterval.Singleton(
                    0.75f * math.dot(direction, direction));
                var radius = MerkabaSphereFlowerAuthority.FloatInterval.Sqrt(
                    MerkabaSphereFlowerAuthority.FloatInterval.Multiply(
                        MerkabaSphereFlowerAuthority.FloatInterval.Multiply(step, step), factor));
                radii[line] = new uint4(math.asuint(radius.Lower), math.asuint(radius.Upper), 0u, 0u);
            }
            tables.AppendVectors(output, "M8FlowerObservedRadius", "float2", radii);
        }
        ...
        private static void AppendChildren(StringBuilder output, GpuTables tables)
        {
            ...
            for (int i = 0; i < MerkabaSphereFlowerAuthority.ChildPetals.Length; i++)
            {
                ...
                rows[i] = new uint4(c.Vertex0, c.Vertex1, c.Vertex2, 0u);
            }
            tables.AppendVectors(output, "M8FlowerChildPetal", "uint3", rows);
        }

        private static void AppendL2Carrier(StringBuilder o, bool csharp, GpuTables tables = null)
        {
            ...
            if (csharp) o.AppendLine("            knots = new L2KnotRule[] {");
            var knotWords = new uint[knots.Length];
            for (int i = 0; i < knots.Length; i++)
            {
                knotWords[i] = knots[i].Source;
                if (csharp)
                {
                    o.Append("                new L2KnotRule(").Append(knots[i].Source).Append("u)");
                    Comma(o, i, knots.Length);
                }
            }
            if (csharp) o.AppendLine("};");
            else tables.AppendScalars(o, "M8FlowerL2KnotSource", "uint", knotWords);
            if (csharp) o.AppendLine("            wedges = new L2WedgeRule[] {");
            var wedgeRows = new uint4[wedges.Length];
            for (int i = 0; i < wedges.Length; i++)
            {
                var w = wedges[i];
                wedgeRows[i] = new uint4((uint)w.Hub | ((uint)w.Ring0 << 9) |
                    ((uint)w.Ring1 << 18), w.Source, 0u, 0u);
                if (csharp)
                {
                    o.Append("                new L2WedgeRule(").Append(w.Hub).Append(", ")
                        .Append(w.Ring0).Append(", ").Append(w.Ring1).Append(", ").Append(w.Source).Append(')');
                    Comma(o, i, wedges.Length);
                }
            }
            if (csharp) o.AppendLine("};");
            else tables.AppendVectors(o, "M8FlowerL2Wedge", "uint2", wedgeRows);
            if (csharp) o.AppendLine("            inverse = new ushort[] {");
            ...
            if (csharp) o.AppendLine("};");
            else AppendUshortTable(o, tables, "M8FlowerL2SourceToWedge", inverse);
            if (csharp) o.AppendLine("            incidenceOffsets = new ushort[] {");
            ...
            if (csharp) o.AppendLine("};");
            else AppendUshortTable(o, tables, "M8FlowerL2IncidenceOffsets", offsets);
            if (csharp) o.AppendLine("            incidenceSources = new uint[] {");
            ...
            if (csharp) o.AppendLine("};");
            else tables.AppendScalars(o, "M8FlowerL2IncidenceSources", "uint", sources);
            if (csharp) o.AppendLine("        }");
            else o.AppendLine(@"
...
uint3 M8FlowerL2WedgeKnots(uint hub,uint wedge)
{
    ...
    return uint3(M8FlowerL2RowKnot(row,0u),M8FlowerL2RowKnot(row,1u+wedge),
        M8FlowerL2RowKnot(row,1u+(wedge+1u)%6u));
            ...
            AppendL2EdgeIncidence(o,csharp,tables);
            ...
            if (csharp)
            {
                o.AppendLine("        private static uint4[] LoadGeneratedL2CarrierBranchMasks() => new uint4[] {");
...
bool M8FlowerPhaseDependents(uint key,out uint4 carriers)
{
    ...
    carriers=M8FlowerPhaseDependentCarriersAt(index);return true;
        ...
        private static void AppendL2EdgeIncidence(StringBuilder o, bool csharp, GpuTables tables)
        {
            ...
            if(csharp)o.AppendLine("};");
            else AppendUshortTable(o,tables,"M8FlowerL2EdgeIncidence",edges);
            if(!csharp)o.AppendLine(@"
...
bool M8FlowerOwnsL2Wedge(uint carrier,uint wedge)
{
    return wedge<6u && (M8FlowerL2CarrierOwnedMask(carrier)&(1u<<wedge))!=0u;
            ...
            var offsets=MerkabaSphereFlowerAuthority.L2WedgeOwnerOffsets;
            var owners=MerkabaSphereFlowerAuthority.L2WedgeOwners;
            o.AppendLine("        private static void LoadGeneratedL2WedgeOwners(out ushort[] offsets, out L2WedgeOwnerRule[] owners) {");
            o.AppendLine("            offsets = new ushort[] {");
            for(int i=0;i<offsets.Length;i++)
            {
                o.Append("    ").Append(offsets[i]);
                Comma(o,i,offsets.Length);
            ...
            o.AppendLine("};");
            o.AppendLine("            owners = new L2WedgeOwnerRule[] {");
            for(int i=0;i<owners.Length;i++)
            {
                var owner=owners[i];
                o.Append("    new L2WedgeOwnerRule(new int3(")
                    .Append(owner.OwnerOffset.x).Append(", ")
                    .Append(owner.OwnerOffset.y).Append(", ")
                    .Append(owner.OwnerOffset.z);
                o.Append("), ").Append(owner.PartnerWedge)
        ...
        private static void AppendPhaseFamilies(StringBuilder o, GpuTables tables)
        {
            ValidatePhaseFamilyRows();
            var families = MerkabaSphereFlowerAuthority.PhaseFamilies;
            Define(o, "M8_FLOWER_PHASE_FAMILY_COUNT", families.Length + "u");
            var rows = new uint4[families.Length];
            for (int i = 0; i < families.Length; ++i)
            {
                var row = families[i];
                rows[i] = new uint4(row.Packed, (uint)row.RootIncidentPetals,
                    (uint)(row.RootIncidentPetals >> 32), 0u);
            }
            tables.AppendVectors(o, "M8FlowerPhaseFamily", "uint4", rows);
            var edges = MerkabaSphereFlowerAuthority.ChildPhaseEdges;
            var words = new uint[edges.Length];
            for (int i = 0; i < edges.Length; ++i)
            {
                words[i] = edges[i].Packed;
            }
            tables.AppendScalars(o, "M8FlowerChildPhaseEdge", "uint", words);
            words = new uint[MerkabaSphereFlowerAuthority.GeometryLevelCount *
            ...
            for (int line = 0; line < MerkabaSphereFlowerAuthority.LineClassCount; ++line)
            {
                ...
                words[level * MerkabaSphereFlowerAuthority.LineClassCount + line] = TableFloatBits(radius);
            }
            tables.AppendScalars(o, "M8FlowerGeometryLoopRadius", "float", words);
            o.AppendLine(@"
...
M8FlowerChildPhaseEdgeRule M8FlowerGetChildPhaseEdge(uint child, uint edge)
{
    ...
    return rule;
        ...
        private static void AppendExcavation(StringBuilder o, GpuTables tables)
        {
            ...
            var rows = new uint4[6];
            for (int face = 0; face < 6; ++face)
            {
                rows[face] = math.asuint(new int4(MerkabaSphereFlowerAuthority.DirtFaceDirection(face), 0));
            }
            tables.AppendVectors(o, "M8FlowerDirtDirection", "int3", rows);
            rows = new uint4[24];
            for (int face = 0; face < 6; ++face)
            for (int corner = 0; corner < 4; ++corner)
            {
                rows[face * 4 + corner] = math.asuint(new int4(MerkabaSphereFlowerAuthority.DirtFaceCorner(face, corner), 0));
            }
            tables.AppendVectors(o, "M8FlowerDirtCorner", "int3", rows);
            o.AppendLine(@"
...
float3 M8FlowerDirtGridPosition(int3 cell, uint face, uint halfFace, uint vertex)
{
    ...
    return float3(M8FlowerDirtGridCoordinate(p.x), M8FlowerDirtGridCoordinate(p.y),
        ...
        private static void AppendSkin(StringBuilder output, GpuTables tables)
        {
            var rows = new uint4[MerkabaSphereFlowerAuthority.SkinChambers.Length];
            for (int i = 0; i < MerkabaSphereFlowerAuthority.SkinChambers.Length;
                 i++)
            {
                ...
                rows[i] = new uint4(value.High, value.Middle, value.Low, packed);
            }
            tables.AppendVectors(output, "M8FlowerSkinChamber", "int4", rows);
            AppendUshortTable(output, tables, "M8FlowerSkinCanonicalToThread",
                MerkabaSphereFlowerAuthority.SkinCanonicalToThread);
            AppendUshortTable(output, tables, "M8FlowerSkinThreadToCanonical",
                MerkabaSphereFlowerAuthority.SkinThreadToCanonical);
            AppendUshortTable(output, tables, "M8FlowerSkinCanonicalL5ToThread",
                MerkabaSphereFlowerAuthority.SkinCanonicalL5ToThread);
            AppendByteTable(output, tables, "M8FlowerSkinL3ChildRank",
                MerkabaSphereFlowerAuthority.SkinL3ChildRank);
            AppendByteTable(output, tables, "M8FlowerSkinL3State",
                MerkabaSphereFlowerAuthority.SkinL3State);
            AppendByteTable(output, tables, "M8FlowerSkinL4ParentThread",
                MerkabaSphereFlowerAuthority.SkinL4ParentThread);
            AppendByteTable(output, tables, "M8FlowerSkinL4ChildRank",
                MerkabaSphereFlowerAuthority.SkinL4ChildRank);
            AppendByteTable(output, tables, "M8FlowerSkinL4State",
                MerkabaSphereFlowerAuthority.SkinL4State);
        ...
        {
            var words = new uint[values.Length];
            for (int i = 0; i < values.Length; i++)
            {
                words[i] = values[i];
            }
            tables.AppendScalars(output, name, "uint", words);
        }

        private static void AppendByteTable(StringBuilder output, GpuTables tables,
        ...
        private static void AppendSectors(StringBuilder output, GpuTables tables)
        {
            ...
            {
                float2 value = MerkabaSphereFlowerAuthority.SectorBoundaries[i].Unit;
                rows[i] = new uint4(TableFloatBits(value.x), TableFloatBits(value.y), 0u, 0u);
            }
            tables.AppendVectors(output, "M8FlowerSectorUnit", "float2", rows);
        }
        ...
        private static void AppendBoundaryRules(StringBuilder output, bool csharp)
        {
            ...
            if (csharp)
            {
                output.AppendLine("        private static BoundaryRule[] LoadGeneratedBoundaryRules() => new BoundaryRule[] {");
                for (int i = 0; i < rules.Length; i++)
                {
                    var r = rules[i];
                    output.Append("            new BoundaryRule(new int4(")
                        .Append(r.Rational.x).Append(',').Append(r.Rational.y).Append(',')
                        .Append(r.Rational.z).Append(',').Append(r.Rational.w).Append(')')
                        .Append(",new int3(").Append(r.D.x).Append(',').Append(r.D.y).Append(',')
            ...
            int tableRows = MerkabaSphereFlowerAuthority.SectorBoundaries.Length;
            output.Append("int4 M8FlowerBoundaryRationalAt(uint index) { return asint(_M8FlowerTables[")
                .Append(tableRows).AppendLine("u+index]); }");
            output.Append("int4 M8FlowerBoundaryIrrationalOwnerAt(uint index) { return asint(_M8FlowerTables[")
                .Append(checked(2 * tableRows)).AppendLine("u+index]); }");
            output.AppendLine(@"
...
");
        ...
        private static void AppendRadicalSectorPatterns(StringBuilder output, bool csharp, GpuTables tables = null)
        {
            ...
            var rows = new uint4[patterns.Length];
            for (int i = 0; i < patterns.Length; i++)
            {
                var value = patterns[i];
                rows[i] = new uint4(value.Incident, value.Positive, value.Negative, 0u);
                if (csharp)
                {
                    output.Append("            new RadicalSectorPattern(")
            ...
            if (csharp) { output.AppendLine("        };"); return; }
            tables.AppendVectors(output, "M8FlowerRadicalSectorPattern", "uint3", rows);
            output.AppendLine(@"
...
bool M8FlowerRadicalSectorSigns(uint node,uint sector,out uint3 signs)
{
    ...
    return true;
        ...
        private static void AppendAnchorSectorPetalMasks(StringBuilder output, bool csharp, GpuTables tables = null)
        {
            for (int kind = 0; kind < 2; kind++)
            {
                ...
                if (csharp) output.Append("        private static ulong[] LoadGeneratedAnchor" + name + "PetalMasks() => new ulong[] {\n");
                var rows = new uint4[masks.Length];
                for (int i = 0; i < masks.Length; i++)
                {
                    if (csharp)
                    {
                    ...
                    rows[i] = new uint4((uint)masks[i], (uint)(masks[i] >> 32), 0u, 0u);
                }
                if (csharp) output.AppendLine("        };");
                else tables.AppendVectors(output, "M8FlowerAnchor" + name + "PetalMask", "uint2", rows);
            }
            if (csharp) return;
            output.AppendLine(@"
...
bool M8FlowerR1SectorFlag(uint node,uint sector,out uint petal)
{
    ...
    return true;
        ...
        private static void AppendJunctionRules(StringBuilder output, bool csharp, GpuTables tables = null)
        {
            var rules = MerkabaSphereFlowerAuthority.JunctionRules;
            if (csharp) output.AppendLine("        private static JunctionRule[] LoadGeneratedJunctionRules() => new JunctionRule[] {");
            var rows = new uint4[rules.Length];
            for (int i = 0; i < rules.Length; i++)
            {
                rows[i] = new uint4(rules[i].Corners, rules[i].Flags, 0u, 0u);
                if (csharp)
                {
                    output.Append("            new JunctionRule(")
                        .Append(rules[i].Corners).Append("u, ").Append(rules[i].Flags).Append("u)");
                    Comma(output, i, rules.Length);
                }
            }
            if (csharp) output.AppendLine("};");
            else tables.AppendVectors(output, "M8FlowerJunctionRule", "uint2", rows);
        }
...
");
        ...
        private static void AppendTetraFrames(StringBuilder output, GpuTables tables)
        {
            ...
            var rows = new uint4[MerkabaSphereFlowerAuthority.TetraFrames.Length];
            for (int i = 0; i < MerkabaSphereFlowerAuthority.TetraFrames.Length; i++)
            {
                rows[i] = math.asuint(MerkabaSphereFlowerAuthority.TetraFrames[i].LineClasses);
            }
            tables.AppendVectors(output, "M8FlowerTetraLine", "int4", rows);
            for (int i = 0; i < MerkabaSphereFlowerAuthority.TetraFrames.Length; i++)
            {
                rows[i] = math.asuint(MerkabaSphereFlowerAuthority.TetraFrames[i].Eta);
            }
            tables.AppendVectors(output, "M8FlowerTetraEta", "int4", rows);
            var words = new uint[MerkabaSphereFlowerAuthority.TetraFrames.Length];
            for (int i = 0; i < MerkabaSphereFlowerAuthority.TetraFrames.Length; i++)
            {
                words[i] = unchecked((uint)MerkabaSphereFlowerAuthority.TetraFrames[i].Chirality);
            }
            tables.AppendScalars(output, "M8FlowerTetraChirality", "int", words);
            const float axis = 0.5773502691896258f;
            rows = new[]
            {
                TableFloatBits(new float3(axis, axis, axis)),
                TableFloatBits(new float3(axis, -axis, -axis)),
                TableFloatBits(new float3(-axis, axis, -axis)),
                TableFloatBits(new float3(-axis, -axis, axis))
            };
            tables.AppendVectors(output, "M8FlowerTetraAxis", "float3", rows);
        }
...
");
        ...
        internal static string BuildObservationProjectionHlsl()
        {
            var o = new StringBuilder(8192);
            o.AppendLine("// GENERATED by MerkabaSphereFlowerCodegen. DO NOT EDIT.");
            o.AppendLine("#ifndef M8_OBSERVATION_PROJECTION_INCLUDED");
            o.AppendLine("#define M8_OBSERVATION_PROJECTION_INCLUDED");
            o.AppendLine("#include \"MerkabaSphereFlower.generated.hlsl\"");
            Define(o, "M8_OBSERVATION_IMAGE_SIDE", "512u");
            o.AppendLine("float4 _M8DepthErrorBounds[2];");
            o.AppendLine(@"
    ...
    out float supportUpperDepth)
{
    ...
    return M8FlowerIsFinite(supportUpperDepth);
        ...
        internal static string BuildDepthCertificateHlsl()
        {
            ...
            o.AppendLine("#include \"../../Runtime/Shaders/MerkabaObservationProjection.generated.hlsl\"");
