...
namespace Genesis.RoomScan.Editor
{
    ...
    public static class MerkabaSphereFlowerCodegen
    {
        ...
        internal static string BuildGeneratedTableBlob()
        {
            ...
            uint4[] canonicalOwners = BuildL2CanonicalOwnerRows();
            uint4[] carriers = BuildL2CarrierRows();
            int ownerRowBase = checked(3 * sectors.Length);
            int carrierRowBase = checked(ownerRowBase + canonicalOwners.Length);
            int rowCount = checked(carrierRowBase + carriers.Length);
            ...
            output.AppendLine("        internal const int SectorBoundsRowOffset = 0;");
            output.Append("        internal const int BoundaryRationalRowOffset = ").Append(sectors.Length).AppendLine(";");
            output.Append("        internal const int BoundaryIrrationalOwnerRowOffset = ").Append(checked(2 * sectors.Length)).AppendLine(";");
            output.Append("        internal const int L2CanonicalOwnerRowOffset = ").Append(ownerRowBase).AppendLine(";");
            output.Append("        internal const int L2CarrierRowOffset = ").Append(carrierRowBase).AppendLine(";");
            output.AppendLine("        internal static uint4[] CreateRows() => new uint4[]");
            output.AppendLine("        {");
            foreach (var sector in sectors)
                AppendTableBlobRow(output, math.asuint(new float4(
                    sector.Enclosure.X.Lower, sector.Enclosure.X.Upper,
                    sector.Enclosure.Y.Lower, sector.Enclosure.Y.Upper)));
            foreach (var rule in rules)
                AppendTableBlobRow(output, math.asuint(rule.Rational));
            foreach (var rule in rules)
                AppendTableBlobRow(output, math.asuint(new int4(rule.D, rule.Owner)));
            foreach (uint4 owner in canonicalOwners) AppendTableBlobRow(output, owner);
            foreach (uint4 carrier in carriers) AppendTableBlobRow(output, carrier);
            output.AppendLine("        };");
            output.AppendLine("    }");
            output.AppendLine("}");
