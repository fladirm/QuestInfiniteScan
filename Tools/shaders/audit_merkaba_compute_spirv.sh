...
if (( kernel_count != 65 )); then
  echo "FAIL: audited $kernel_count kernels; expected 65" >&2
  exit 1
fi

echo "PASS: 65 Quest compute kernels validate; writable buffer/image storage <= 8; no RW/read alias pair"
