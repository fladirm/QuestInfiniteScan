...
if (( kernel_count != 55 )); then
  echo "FAIL: audited $kernel_count kernels; expected 55" >&2
  exit 1
fi

echo "PASS: 55 Quest compute kernels validate; writable buffer/image storage <= 8; no RW/read alias pair"
