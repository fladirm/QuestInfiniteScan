...
if (( kernel_count != 70 )); then
  echo "FAIL: audited $kernel_count kernels; expected 70" >&2
  exit 1
fi

echo "PASS: 70 Quest compute kernels validate; writable buffer/image storage <= 8; no RW/read alias pair"
