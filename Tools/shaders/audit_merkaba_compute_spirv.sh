...
if (( kernel_count != 68 )); then
  echo "FAIL: audited $kernel_count kernels; expected 68" >&2
  exit 1
fi

echo "PASS: 68 Quest compute kernels validate; writable buffer/image storage <= 8; no RW/read alias pair"
