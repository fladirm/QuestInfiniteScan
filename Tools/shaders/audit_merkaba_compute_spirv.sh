...
if (( kernel_count != 76 )); then
  echo "FAIL: audited $kernel_count kernels; expected 76" >&2
  exit 1
fi

echo "PASS: 76 Quest compute kernels validate; writable buffer/image storage <= 8; no RW/read alias pair"
