...
if (( kernel_count != 71 )); then
  echo "FAIL: audited $kernel_count kernels; expected 71" >&2
  exit 1
fi

echo "PASS: 71 Quest compute kernels validate; writable buffer/image storage <= 8; no RW/read alias pair"
