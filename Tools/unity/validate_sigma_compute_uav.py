#!/usr/bin/env python3
"""Reject Sigma compute kernels that exceed Quest Vulkan's eight UAV slots."""
...
DEFAULT_SHADER_ROOT = ROOT / "Runtime" / "Resources" / "SigmaPrism"
DEFAULT_LIMIT = 8
...
def main() -> int:
    ...
    if failures:
        print("Sigma compute UAV validation failed:", file=sys.stderr)
        print("\n".join(f"- {failure}" for failure in failures), file=sys.stderr)
        return 1
    print(f"Sigma compute UAV validation passed (limit={args.limit}).")
    return 0
