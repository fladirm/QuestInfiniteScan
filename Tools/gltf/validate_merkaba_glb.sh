...
ulimit -n 65536
if (( $# > 1 )); then
  echo "usage: $0 [/exact/path/to/file.glb]" >&2
  exit 2
fi

if (( $# == 1 )); then
  GLB_PATH="$(readlink -f -- "$1")"
  test -s "${GLB_PATH}"
else
  FIXTURE_DIR="${QIS_BUILD_ROOT}/TestResults"
  GLB_PATH="${FIXTURE_DIR}/merkaba-fixture.glb"
  UNITY_LOG="${FIXTURE_DIR}/merkaba-glb-fixture.log"
  mkdir -p "${FIXTURE_DIR}"
  export QIS_MERKABA_GLB_FIXTURE_PATH="${GLB_PATH}"
...
node "${NPM_WORK}/verify_merkaba_glb.mjs" "${GLB_PATH}"
