#!/usr/bin/env python3
"""Generate deterministic file/type/function/GPU/DAG maps for Sigma-PRISM-16."""
...
TASK_GLOBS = {
    "S4-00": ("new_spec.md", "AGENTS.md", ".codex/*", "README.md",
               "ALGORITHM.md", "Editor/*", "Runtime/Core/*", "Runtime/UI/*",
               "Runtime/SigmaPrism/Capture/*", "Runtime/SigmaPrism/Calibration/*",
               "*SigmaRigBridge*", "Tools/unity/*"),
    "S4-01": ("*SigmaNumericDomain*", "*Sedenion16*", "*SigmaOperator*",
               "Tools/sigma/*"),
    "S4-02": ("*SigmaCarrier*", "*CarrierPage*", "*CarrierCodec*", "*ExactCodec*"),
    "S4-03": ("*ForwardReadout*", "*SigmaPrediction*", "*SigmaRigBridge*"),
    "S4-04": ("*InverseDepth*", "*DepthCell*", "*AdmissibleCell*", "*ProjectiveCommit*"),
    "S4-05": ("*Singular*", "*Annihilator*", "*Associator*", "*Transition*"),
    "S4-06": ("*InverseRgb*", "*RgbCell*", "*ConstraintCertificate*"),
    "S4-07": ("*GaugeRefine*", "*CarrierGauge*"),
    "S4-08": ("*PoseGauge*", "*PoseCell*"),
    "S4-09": ("*SceneEvolution*", "*Transport*", "*Exclusion*", "*Probation*"),
    "S4-10": ("*CarrierPager*", "*SigmaPersistence*", "*SigmaSnapshot*", "*CertificateStore*"),
    "S4-11": ("*Meshlet*", "*ReadoutRender*", "*ViewCull*", "*HiZ*"),
    "S4-12": ("*Appearance*", "*Pbr*", "*PBR*", "*Glb*", "*GLB*"),
    "S4-13": ("*Acceptance*", "*Benchmark*", "*Diagnostics*", "*Telemetry*"),
}
